...
////////ae/ecc-counter-modifiedCriticalPath//////Atena
...
//Having DetectionOnly and Correction options (Atena)
  ...
  // Add the signal to control correction phase
  logic [3:0] correction_start_index;
  logic [3:0] correction_index; ///index generated by the counter module  this selects which entry to check each cycle
  logic correction_enable;
  logic load_correction_start;

  ///////////////// FSM States for correction controller(Atena)
  typedef enum logic [1:0]{
    IDLE,
    CORRECTING
  } corr_state_e;

  corr_state_e  corr_state_q, corr_state_d;


  /////////////////////// FSM: next state logic(Atena)
  always_comb begin
    corr_state_d = corr_state_q;
    ...
    case(corr_state_q)
        ...
        CORRECTING: begin
          ...
          /////(2nd solution) start counting from smallest index and continue till last corrupted and then go to IDLE;
          /////but if an entry be corrupted which has already been passed, counter continue counting and wrapping to reach the corrupted one
    ...
    //////////////////////////////// FSM: state register(Atena)
    always_ff @(posedge clk_i or negedge rst_ni) begin
      if (!rst_ni) begin
           corr_state_q <= IDLE;
      end else begin
        corr_state_q <= corr_state_d;
      end
    end

////////////////////////Check for 1-bit error and determine smallest index (Atena)

always_comb begin
    correction_enable = 1'b0;
    correction_start_index = 0;
...
counter #(
  .WIDTH($clog2(TLB_ENTRIES))
) i_counter(
  .clk_i   (clk_i),
  .rst_ni  (rst_ni),
  .clear_i   (1'b0),
  .en_i    (corr_state_q == CORRECTING),
  .load_i  (load_correction_start),
  .d_i     (correction_start_index),
  .q_o (correction_index),
  .overflow_o(), // left unconnected
  .down_i(1'b0)  // tied to 0
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    //////////////////////////////////////////////////////////////////////////////////////
    /////////////////////////// Also in case of having 1 bit error in currently used entry, the entry  will be invalidated(Atena)
    //for (genvar i = 0; i < TLB_ENTRIES; i++) begin

      //logic entry_invalid;
      //assign entry_invalid = (DetectionOnly &&
                            //(|invalidate_valid || |invalidate_tag[i] || |invalidate_pte[i] || |invalidate_gpte[i])) ||
                            //(invalidate_valid[1]) ||
                            //(invalidate_tag[i][1]) ||
                            //(invalidate_pte[i][1]) ||
                            //(invalidate_gpte[i][1]) ||
                            //((invalidate_tag[i] == 2'b01) && entry_used_by_mmu[i]);

      //assign tags[i].valid = entry_invalid ? 1'b0 : valid_dec[i];

      //assign tlb_content_q[i].pte = entry_invalid ?
                                  //riscv::pte_t'(content_q[i].pte) :
                                  //riscv::pte_t'(tlb_content_dec[i].pte);

      //assign tlb_content_q[i].gpte = entry_invalid ?
                                   //riscv::pte_t'(content_q[i].gpte) :
                                   //riscv::pte_t'(tlb_content_dec[i].gpte);

      //assign tags[i].tag = entry_invalid ?
                         //tags_q[i] :
                         //partial_tags_t'(tags_dec[i]);
...
//////////////////////////////////////////////
    ...
    for (int unsigned i = 0; i < TLB_ENTRIES; i++) begin
      ...
      if (flush_i) begin
        ...
        // normal replacement
        ////changing the condition for criticalpath modification (Atena)
      end else if (update_i.valid & replace_en[i] && !(lu_hit[i] && invalidate_tag[i] == 2'b01 && !DetectionOnly)) begin
        // update tag array
        tags_n[i] = tags_enc;
        valid_update[i] = 1'b1;
        // and content as well
      ...
      // invalidate currently used entry with 1-bit error_criticalpath modification(Atena)
          else if (!DetectionOnly && lu_hit[i] && invalidate_tag[i] == 2'b01) begin
                  valid_update[i] = 1'b0;
                  $display("[ECC] Entry %0d has 1-bit error and was used  invalidated at %0t", i, $time);
          end
      // Correct single-bit errors in TLB tags
      //////(for updating tags_n in case of having 1 bit error in not used entry_Atena)
      ////changing the condition for criticalpath modification (Atena)
          else if(!DetectionOnly && (i == correction_index) && invalidate_tag[i] == 2'b01 && !lu_hit[i])begin
